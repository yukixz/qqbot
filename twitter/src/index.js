...
async function init() {
  ...
  last_id = timeline[0].id + 1
  log(`Last timeline id: ${last_id}`)
}

async function broadcast() {
  log(`Broadcast since ${last_id}`)
  const timeline = await fetch(last_id)
  last_id = timeline[0].id + 1

  for (const tweet of timeline.reverse()) {
    log(`Broadcasting ${tweet.id} @${tweet.user.screen_name}`)
